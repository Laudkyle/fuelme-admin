...
export default function Agents() {
  ...
  const validateForm = () => {
    ...
    if (!formData.station_uuid.trim()) newErrors.station_uuid = "Station selection is required";
  ...
  // Reset form data
  const resetForm = () => {
    setFormData({ fullname: "", phone: "", station_uuid: "", transaction_pin: "" });
    setErrors({});
    setEditMode(false);
  ...
  const columns = [
    ...
    { name: "Phone", selector: (row) => row.phone, sortable: true },
    {
      name: "Station",
      selector: (row) => {
        const station = stations.find((s) => s.station_uuid === row.station_uuid);
    ...
    { name: "Transaction PIN", selector: (row) => row.transaction_pin, sortable: true },
    {
      ...
      cell: (row) => (
        <div className="flex">
          ...
          <button onClick={() => handleDelete(row.agent_uuid)} className="text-red-500">Delete</button>
        </div>
      ),
    },
  ];
  ...
  return (
    <div className="p-6">
      ...
      <button onClick={() => { resetForm(); setIsModalOpen(true); }} className="mb-4 px-4 py-2 bg-blue-500 text-white rounded">
        + Add Agent
      </button>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-30 flex justify-center items-center">
          <div className="bg-white p-6 rounded shadow-md w-96">
            <h2 className="text-lg font-bold mb-4">{editMode ? "Edit Agent" : "Add Agent"}</h2>
